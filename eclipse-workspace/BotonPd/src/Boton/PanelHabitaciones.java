    ...
    private void mostrarDetallesHabitacion(Habitacion habitacion) {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            if (!nuevoEstado.equals(habitacion.getEstado())) {
                ...
                if (exito) {
                    // SI EL NUEVO ESTADO ES MANTENIMIENTO, CREAR REGISTRO AUTOMÁTICAMENTE
                    if (nuevoEstado.equals("MANTENIMIENTO")) {
                        // Crear un registro de mantenimiento con la fecha actual
                        ...
                        boolean registroCreado = db.agregarMantenimiento(nuevoMantenimiento);
                        
                        if (registroCreado) {
                            JOptionPane.showMessageDialog(this, 
                                "Estado actualizado correctamente.\nSe ha creado un registro de mantenimiento para la habitación " + 
                                habitacion.getNumero() + ".");
                        } else {
                            JOptionPane.showMessageDialog(this, 
                                "Estado actualizado, pero hubo un problema al crear el registro de mantenimiento.", 
                                "Advertencia", 
                                JOptionPane.WARNING_MESSAGE);
                        }
                    } 
                    // SI EL ESTADO ANTERIOR ERA OCUPADA Y EL NUEVO ES DISPONIBLE, ELIMINAR PAGOS
                    else if (habitacion.getEstado().equals("OCUPADA") && nuevoEstado.equals("DISPONIBLE")) {
                        db.eliminarPagosPorNumeroHabitacion(habitacion.getNumero());
                        JOptionPane.showMessageDialog(this, 
                            "Estado actualizado correctamente.\nSe eliminaron los pagos asociados a la habitación " + 
                            habitacion.getNumero() + ".");
                    } 
                    else {
