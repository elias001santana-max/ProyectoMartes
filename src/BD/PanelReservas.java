...
public class PanelReservas extends JPanel {
	...
	private void nuevaReserva() {
		// Crear diálogo simple para nueva reserva
	...
	private void eliminarReserva() {
		...
		int id = (int) table.getValueAt(selectedRow, 0); // ID is in column 0
	...
	private void editarReserva() {
		...
		JPanel formPanel = new JPanel(new GridLayout(8, 2, 10, 10)); // Increased rows to 8
		formPanel.setBorder(new EmptyBorder(20, 20, 20, 20));
		formPanel.setBackground(Color.WHITE);
		...
		btnGuardar.addActionListener(e -> {
			try {
				Reserva actualizada = new Reserva(
					...
					LocalDate.parse(txtEntrada.getText()),
					LocalDate.parse(txtSalida.getText()),
					(String) comboEstado.getSelectedItem(), // Use selected status
					Double.parseDouble(txtPrecio.getText())
				);
